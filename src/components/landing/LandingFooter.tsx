...
export async function LandingFooter() {
  ...
  const linkClass = 'text-sm text-[#A0A0AB] transition-colors hover:text-white'
  const colTitleClass = 'mb-[3px] text-[12.5px] uppercase tracking-[0.08em] text-[#5C5C66]'
    ...
    <footer className="relative z-[1] border-t border-white/[0.07]">
      <div className="mx-auto flex max-w-[1200px] flex-wrap items-start justify-between gap-10 px-7 py-12">
        <div className="max-w-[280px]">
          ...
          <p className="text-sm leading-[1.6] text-[#7E7E89]">{t('tagline')}</p>
        </div>
      ...
      <div className="border-t border-white/[0.06]">
        <div className="mx-auto flex max-w-[1200px] flex-wrap items-center justify-between gap-4 px-7 py-[22px]">
          <span className="text-[13px] text-[#5C5C66]">
            {t('copyright', { year: new Date().getFullYear() })}
          </span>
          <span className="font-geist-mono text-xs text-[#5C5C66]">{t('operational')}</span>
        </div>
      </div>
    </footer>
  )
