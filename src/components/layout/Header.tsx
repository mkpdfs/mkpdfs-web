...
const ghostControl =
  'flex h-[34px] items-center justify-center gap-[7px] rounded-[9px] border border-ink/10 bg-transparent text-[13px] font-medium text-fg-muted transition-colors hover:bg-ink/[0.05] hover:text-fg'
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {isOpen && (
        <div
          role="listbox"
          className="absolute right-0 top-full z-50 mt-1.5 min-w-[140px] rounded-[10px] border border-ink/10 bg-surface-card py-1 shadow-[0_16px_40px_rgba(0,0,0,0.6)]"
        >
          {locales.map((loc) => (
            <button
              key={loc}
              ...
              className={cn(
                ...
                loc === locale
                  ...
                  : 'text-fg-muted hover:bg-ink/[0.05] hover:text-fg'
              )}
            >
              {localeNames[loc]}
            </button>
...
export function Header() {
  ...
  return (
    <header className="flex h-[60px] shrink-0 items-center gap-2 border-b border-ink/[0.07] px-4 sm:px-7">
      {/* Mobile menu button */}
      <button
        type="button"
        className="-m-2 p-2 text-fg-muted hover:text-fg lg:hidden"
      ...
      <div className="flex flex-1 items-center justify-end gap-2">
        ...
        <div className="mx-1.5 hidden h-5 w-px bg-ink/10 sm:block" />
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 z-50 w-full max-w-xs overflow-y-auto border-r border-ink/[0.07] bg-surface px-5 py-5">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                className="-m-2 rounded-md p-2 text-fg-muted hover:text-fg"
                onClick={() => setMobileMenuOpen(false)}
              >
            ...
            <div className="mt-6 flex items-center gap-2 border-t border-ink/[0.07] pt-5">
              <div className="flex h-[30px] w-[30px] items-center justify-center rounded-full bg-[linear-gradient(140deg,#8C6CFF,#5B3FE0)] text-[13px] font-semibold text-white">
                {initial}
              </div>
              <PortalLanguageSelector />
