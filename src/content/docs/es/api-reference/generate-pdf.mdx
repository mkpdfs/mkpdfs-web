...
| Encabezado | Valor |
|---|---|
| `x-api-key` | `tlfy_...` — obligatorio. Consulta [Autenticación](/es/docs/api-reference/authentication). |
...
| `async` | boolean | No | Envía para procesamiento en segundo plano. Por defecto `false`. Cuando es `true`, la API responde de inmediato y envía el PDF por correo cuando esté listo. |
...
Si tu cuenta no tiene créditos suficientes, la API devuelve `402 INSUFFICIENT_CREDITS`. Consulta [Errores](/es/docs/api-reference/errors) o visita el [panel](https://mkpdfs.com/billing) para comprar créditos.
...
Consulta la página de [Errores](/es/docs/api-reference/errors) para la lista completa. Casos comunes:
