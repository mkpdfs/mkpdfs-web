...
Edit the `.hbs` file locally, then push it back — `push` takes only the file;
the template it belongs to is remembered in `.mkpdfs.json` (written by `pull`),
or force a target with `--id`:
